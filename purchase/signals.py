...
def update_vendor_performance(sender, instance: PurchaseOrder, **kwargs):
    ...
    if old_instance.status != instance.status:
        ...
        old_total_completed_order = PurchaseOrder.objects.filter(
            status=StatusChoice.COMPLETED, vendor_id=vendor.pk
        ).count()  # 3
        old_total_on_time_completed_order = (
            old_average_on_time_delivery_rate * old_total_completed_order
        )  # 3
        is_current_order_on_time = (
            True if old_instance.delivery_date >= instance.delivery_date else False
        )
        new_total_completed_order = old_total_completed_order + 1
        new_total_on_time_completed_order = (
            old_total_on_time_completed_order + 1
            if is_current_order_on_time
            else old_total_on_time_completed_order
        )
        new_average_on_time_delivery_rate = (
            new_total_on_time_completed_order / new_total_completed_order
        )
    ...
    if instance.quality_rating != old_instance.quality_rating:
        ...
        old_rating_count = (
            PurchaseOrder.objects.filter(
                vendor_id=vendor.pk, status=StatusChoice.COMPLETED
            )
        ...
        current_quality_rating_average = (
            current_quality_rating / current_quality_rating_count
        )
...
def update_vendor_average_response_time(sender, instance: PurchaseOrder, **kwargs):
    ...
    if instance.acknowledgment_date != old_instance.acknowledgment_date:
        ...
        current_response_time = (
            instance.acknowledgment_date - issue_date
        ).total_seconds() / 3600
        total_orders = PurchaseOrder.objects.filter(
            vendor_id=vendor.pk, acknowledgment_date__isnull=False
        ).count()
        new_average_response_time = (
            current_response_time + old_average_response_time * total_orders
        ) / (total_orders + 1)
...
def update_vendor_fulfillment_rate(sender, instance: PurchaseOrder, **kwargs):
    ...
    if old_instance.status != instance.status:
        completed_order_count = PurchaseOrder.objects.filter(
            status=StatusChoice.COMPLETED, vendor_id=vendor.pk
        ).count()
